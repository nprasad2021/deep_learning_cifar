...
#X_train = 2*(X_train - X_train.min()) / (X_train.max() - X_train.min())
...
model.fit(X_train, y_train, batch_size=100, epochs=30, validation_split=0.1,  shuffle=True)
...
prediction_test_df.to_csv("submission5.csv", index_label='Id')
model.save_weights("resNetV5")
